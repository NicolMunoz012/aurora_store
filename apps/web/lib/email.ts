...
const FROM = process.env.RESEND_FROM ?? "Aurora <noreply@aurorabelleza.com>";
...
export async function sendPasswordResetEmail(
  ...
  await resend.emails.send({
    from: FROM,
    to,
    subject: "Restablece tu contraseña — Aurora",
    html: `
      <div style="font-family: Georgia, serif; max-width: 480px; margin: 0 auto; padding: 32px;">
        ...
        <p style="color: #666; font-size: 15px; line-height: 1.6; margin-bottom: 24px;">
          Recibimos una solicitud para restablecer la contraseña de tu cuenta en Aurora.
        </p>
        <a href="${resetUrl}"
        ...
        <p style="color: #bbb; font-size: 12px;">Aurora · Colombia</p>
      </div>
    `,
  });
}
